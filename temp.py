...
#plot mx+b given k, scalar = 2400
def lineplot(m, k):
    plt.plot([min(xdata),1990],[k,m*(1990-min(xdata)) + k])
...
minm = 0
mink = 0
for k in range(min(ydata) + 1, max(ydata)/6):
    #print(k)
    #print(fmin(k))
    #print(fmax(k))
    for m in range(int(fmin(k)), int(fmax(k))-4):
        if(error(m, k) < minerr):
            minerr = error(m, k)
            minm = m
            mink = k
